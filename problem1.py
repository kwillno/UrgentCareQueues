...
def getCI(lam, mu, percentage=0.95):
    ...
    print(
        f"Wait times {percentage*10:.0f}% CI: {CI[0]*60:.2f} - {CI[1]*60:.2f} mins, {CI[0]:.4f} - {CI[1]:.4f} hours"
    )
    ...
    for i in range(1, timeslices):
        ...
        if not (i < block):
            ...
            change = np.random.choice(
                [-1, 1], p=(mu_i / (mu_i + lam), lam / (mu_i + lam))
            )

            if (  # All the possible situations for change in N(t)
                U[i - 1] == 0 and change == -1
            ) or (np.random.uniform() > p and change == 1):
                N[i] += change
            else:
                U[i] += change
...
def getCI_UN(p, lam, mu, percentage=0.95):
    ...
    print("Normal distribution")
    CI_U = st.norm.interval(
        percentage, loc=np.mean(waitTimesU), scale=st.sem(waitTimesU)
    )
    print(
        f"Wait times {percentage*10:.0f}% CI Urgent: {CI_U[0]*60:.2f} - {CI_U[1]*60:.2f} mins, {CI_U[0]:.4f} - {CI_U[1]:.4f} hours"
    )

    CI_N = st.norm.interval(
        percentage, loc=np.mean(waitTimesN), scale=st.sem(waitTimesN)
    )
    print(
        f"Wait times {percentage*10:.0f}% CI Normal: {CI_N[0]*60:.2f} - {CI_N[1]*60:.2f} mins, {CI_N[0]:.4f} - {CI_N[1]:.4f} hours"
    )
